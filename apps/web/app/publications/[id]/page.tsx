...
export default function PublicationDetailPage({ params }: { params: { id: string } }) {
  ...
  const handleSubmitComment = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/publications/${params.id}/comments`, {
        method: 'POST',
        headers: {
      ...
      if (result.success) {
        setNewComment("");
        fetchPublication(); // Refresh to get updated comments
      } else {
      ...
      <>
        ...
        <div className="min-h-screen bg-vidwanic-background flex items-center justify-center">
          <div className="text-center">
            <div className="animate-spin rounded-full h-32 w-32 border-b-2 border-vidwanic-orange mx-auto mb-4"></div>
            <p className="text-vidwanic-text text-lg">Loading publication...</p>
          </div>
        </div>
      </>
    );
              ...
              <div>
                ...
                <div className="bg-white rounded-xl p-6 shadow-lg mb-8">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                    ...
                    <div>
                      <div className="text-2xl font-bold text-vidwanic-text">{publication._count.comments}</div>
